...
func (h *Handler) adminGetSurvey(c *gin.Context) {
	...
	module, err := h.usecase.Modules.GetById(c.Request.Context(), id)
	if err != nil {
...
func (h *Handler) adminCreateOrUpdateSurvey(c *gin.Context) {
	...
	if err := h.usecase.Surveys.Create(c.Request.Context(), usecase.CreateSurveyInput{
		ModuleID: id,
		SchoolID: school.ID,
...
func (h *Handler) adminDeleteSurvey(c *gin.Context) {
	...
	if err := h.usecase.Surveys.Delete(c.Request.Context(), school.ID, id); err != nil {
...
func (h *Handler) adminGetSurveyResults(c *gin.Context) {
	...
	results, count, err := h.usecase.Surveys.GetResultsByModule(c.Request.Context(), id, &query)
	if err != nil {
		newResponse(c, http.StatusInternalServerError, "failed to delete survey")
...
func (h *Handler) adminGetSurveyStudentResults(c *gin.Context) {
	...
	results, err := h.usecase.Surveys.GetStudentResults(c.Request.Context(), moduleId, studentId)
	if err != nil {
		newResponse(c, http.StatusInternalServerError, "failed to delete survey")
