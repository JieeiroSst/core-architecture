...
func (h *Handler) studentSignUp(c *gin.Context) {
	...
	if err := h.usecase.Students.SignUp(c.Request.Context(), usecase.StudentSignUpInput{
		Name:         inp.Name,
		Email:        inp.Email,
...
func (h *Handler) studentSignIn(c *gin.Context) {
	...
	res, err := h.usecase.Students.SignIn(c.Request.Context(), usecase.SchoolSignInInput{
		SchoolID: school.ID,
		Email:    inp.Email,
		Password: inp.Password,
	})
...
func (h *Handler) studentRefresh(c *gin.Context) {
	...
	res, err := h.usecase.Students.RefreshTokens(c.Request.Context(), school.ID, inp.Token)
	if err != nil {
		if errors.Is(err, domain.ErrStudentBlocked) {
			newResponse(c, http.StatusForbidden, err.Error())
...
func (h *Handler) studentVerify(c *gin.Context) {
	...
	if err := h.usecase.Students.Verify(c.Request.Context(), code); err != nil {
...
func (h *Handler) studentGetModuleContent(c *gin.Context) {
	...
	content, err := h.usecase.Students.GetModuleContent(c.Request.Context(), school.ID, studentId, moduleId)
	if err != nil {
...
func (h *Handler) studentSubmitSurvey(c *gin.Context) {
	...
	if err := h.usecase.Surveys.SaveStudentAnswers(c.Request.Context(), usecase.SaveStudentAnswersInput{
		StudentID: studentId,
		SchoolID:  school.ID,
		ModuleID:  moduleId,
		Answers:   answers,
...
func (h *Handler) studentSetLessonFinished(c *gin.Context) {
	...
	if err := h.usecase.Students.SetLessonFinished(c.Request.Context(), studentId, lessonId); err != nil {
		if errors.Is(err, domain.ErrModuleIsNotAvailable) {
			newResponse(c, http.StatusForbidden, err.Error())
...
func (h *Handler) studentGetModuleOffers(c *gin.Context) {
	...
	offers, err := h.usecase.Offers.GetByModule(c.Request.Context(), school.ID, moduleId)
	if err != nil {
		newResponse(c, http.StatusInternalServerError, err.Error())
...
func (h *Handler) studentCreateOrder(c *gin.Context) {
	...
	id, err := h.usecase.Orders.Create(c.Request.Context(), studentId, offerId, promoId)
	if err != nil {
		switch err {
...
func (h *Handler) studentGeneratePaymentLink(c *gin.Context) {
	...
	url, err := h.usecase.Payments.GeneratePaymentLink(c.Request.Context(), orderId)
	if err != nil {
...
func (h *Handler) studentGetAccount(c *gin.Context) {
	...
	student, err := h.usecase.Students.GetById(c.Request.Context(), school.ID, studentId)
	if err != nil {
		newResponse(c, http.StatusInternalServerError, err.Error())
