...
func (h *Handler) adminUploadImage(c *gin.Context) { //nolint:funlen
	...
	url, err := h.usecase.Files.UploadAndSaveFile(c.Request.Context(), domain.File{
		SchoolID:    school.ID,
		Type:        domain.Image,
...
func (h *Handler) adminUploadVideo(c *gin.Context) { //nolint:funlen
	...
	if _, err := io.Copy(f, bytes.NewReader(buffer)); err != nil {
		if err := h.usecase.Files.UpdateStatus(c.Request.Context(), tempFilename, domain.ClientUploadError); err != nil {
	...
	if rangeInfo.initialUploadRequest() {
		id, err := h.usecase.Files.Save(c.Request.Context(), domain.File{
			Type:            domain.Video,
	...
	if rangeInfo.isUploadCompleted() {
		if err := h.usecase.Files.UpdateStatus(c.Request.Context(), tempFilename, domain.UploadedByClient); err != nil {
